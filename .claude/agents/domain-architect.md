...
## features/*.feature — Gherkin Business Rules

The complete Gherkin DSL reference — tags, operators, conditions, metadata, outcomes, validation rules, common mistakes — lives in the **appget-feature-dsl** skill (`.claude/skills/appget-feature-dsl/`). Load and reference it for all syntax details.
...
1. Every `@target` references a model/view in schema.sql/views.sql
2. Every field in `When` conditions verified against the exact target's columns
3. For `@view` targets: field must exist in `SELECT` clause, not just WHERE/JOIN
4. Every metadata category and field matches metadata.yaml
5. No DATE/TIMESTAMP/DATETIME columns, no field-to-field comparisons, no tautological ORs
