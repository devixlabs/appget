...
Step 1: compileGenerators (independent)
        ├─ Compiles: FeatureToSpecsConverter, SQLSchemaParser, SchemaToProtoConverter,
...
Step 3: generateProto (depends on featuresToSpecs + parseSchema)
        ├─ Runs: SchemaToProtoConverter + protoc (SQL + specs → .proto → Java)
...
### SchemaToProtoConverter + protoc: SQL → .proto → Java Models + Views
...
**Input**: `.proto` files (generated by SchemaToProtoConverter)
...
**Generated Components** (26 files total):

**Per-Model (5 × 3 = 15 files)**:
...
- `SchemaToProtoConverter.java` - SQL + specs → .proto file generation
...
Generators (`SQLSchemaParser`, `SchemaToProtoConverter`, `SpecificationGenerator`, etc.) create static final logger fields:
```java
...
| `src/main/java/dev/appget/codegen/SchemaToProtoConverter.java` | SQL + specs → .proto files (models, views, services, rules) |
