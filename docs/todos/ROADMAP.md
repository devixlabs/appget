...
- **What**: Generated servers respond with HTML, JSON, or XML based on the `Accept` request header. Transform logic behind a `ContentTransform` interface, framework wiring generated by each emitter.
- **Why**: Browsers get server-rendered HTML (old-fashioned GET/POST, no JS). API clients get JSON. Legacy systems get XML. Same endpoints, same controllers, no proxies.
- **Blocked by**: Research (BLOCKING) — must survey existing libraries/plugins per framework before building from scratch. Jeff (human) owns framework-specific research; Claude owns broad survey.
- **Effort**: Medium (if existing library found) to Large (if built from scratch)
- **Note**: The `ContentTransform` interface must be abstract/swappable so implementations can be replaced with third-party libraries if better solutions are found during research.

### Suggested order: ~~0a~~ → ~~0b~~ → 0d → 0f → 0e → 0c

0a and 0b are done. 0d (HTML static) is done. 0f (content negotiation) makes the HTML live with real data — requires research first. 0e (multi-industry) validates everything. 0c (gRPC/GraphQL) deferred until REST pipeline is proven.
